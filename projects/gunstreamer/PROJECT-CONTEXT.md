...
| **C. Legacy 184K Wave Launch** | 184K BBGV-cross-pollinated cold contacts — **NOT a real asset.** Per Troy (2026-05-21): these are unengaged BBGV leftovers, bounce magnets, or chronic non-responders. Mailing further waves is a sender-reputation hazard, not a growth lever. Treat as dead unless a clean re-permission flow is built. | — | Klaviyo, separate account | Wave 1 already sent (10K). **STOP. Do not send waves 2-18.** `CONTENT-GROWTH-STRATEGY-2026.md` Phase 1 is stale on this point and needs revision. |
...
|---|---|---|---|
| 2026-05-28 | **Live-Event Ads — strategy, agency vs internal-team reframe, scale architecture** | Designed the full paid-advertising playbook for GunStreamer's weekly dealer live events. Locked Meta firearm-policy doctrine: zero firearm language/imagery in ad creative, destination URL, or LP — dealer's customers fill in implicit context (the "could this be a barbershop or brewery's live event ad?" test). Funnel: cold ad (Meta + Google, 5-15 mi geo) → LP form (phone + email) → Klaviyo SMS reminder sequence (T-24h, T-2h, T-15m) → live stream → off-platform FFL sale. Corrected operating model: this is NOT agency-for-hire — GunStreamer is the buyer, dealers are inventory, recoup is via existing 5% GMV commission. No retainer, no MSA, no dealer-side invoicing. Spend funded 100% by GunStreamer on a dedicated corporate card. **Lazy-dealer setup locked:** dealer fills 10-min intake form + taps ONE Page admin notification — that's it. GunStreamer owns all BMs, ad accounts, Pixels, LPs, payment cards. **Multi-BM rotation pattern (3 BMs in pilot) for ban-contagion containment** — single BM ban contained to ~4 dealers, not the whole portfolio. **Scale architecture for 1→500 dealers:** Tier 1 (1-3 manual) → Tier 2 (4-15 templated) → Tier 3 (16-50 semi-automated) → Tier 4 (50-200 platform) → Tier 5 (200+ productized). Identified the strategic fork: Path 1 (services, linear cost) vs Path 2 (platform, logarithmic cost). Path 2 is the only viable option past ~50 dealers. **Architecture principle: build for 500 from dealer #1** — Supabase + Next.js LP engine + multi-BM rotation, no duct tape. Strategic reframe: the long-term product isn't "we run ads for gun shops" — it's a turnkey local customer acquisition engine for any restricted vertical (cannabis, CBD, fireworks, vape, sports betting). | New artifacts in this repo: `projects/gunstreamer/docs/live-event-ads-playbook.md` (full strategy + scale architecture) + `projects/gunstreamer/Session-Start-Prompts-Live-Event-Ads-2026-05-28.md` (3 paste-ready prompts for parallel sessions: ad architecture build, dealer kit vendor sourcing, GHL ↔ LP wiring). Session log: `sessions/2026-05-28-claude-code-gunstreamer-live-event-ads-scale-architecture.md`. **Differs from the 2026-05-27 3-workstream prompts** — those covered the same 3 streams (FB Ads, Kit, GHL) but at a tactical/build level. This session's prompts assume the live-event ads scale-architecture framing and build foundations for the 500-dealer end state. Open threads added to §11: (a) apply for Meta Verified Business status NOW (2-4 week clock), (b) open dedicated Ramp/Brex card for ad spend separate from main ops, (c) pick 2 employee/co-founder personal FB accounts to own BM-2 and BM-3, (d) subdomain decision (`[dealer].glive.com` neutral domain recommended over `gunstreamer.com` for ad LPs — gunstreamer.com domain triggers Meta firearm-policy scrutiny). |
| 2026-05-28 | **GHL Dealer CRM design + three-workstream split** | Designed the GunStreamer GHL subaccount as the dealer CRM. Produced full setup playbook (`Projects/GunStreamer/GHL-Dealer-CRM-Setup-Playbook-2026-05-27.md` in Obsidian), simplified the dealer pipeline from 10 stages to **6 stages** (Prospect → Onboarding Zoom Scheduled → Dealer Kit Sent → Demo Scheduled → Streamed Once → Streaming Weekly), wrote click-by-click workflow spec (`Projects/GunStreamer/GHL-Workflow-Spec-Dealer-Form-Inbound-2026-05-28.md`), analyzed the dealer-contact landing page stack (static HTML + Tailwind + vanilla JS — scaffolding, not prod), locked the form-to-CRM architecture (form `onSubmit` → Vercel API route `/api/dealer-signup` → GHL Inbound Webhook → Stage 1 Prospect). Form fields locked from screenshots: Shop Name, Email, Phone (step 1) + FFL # `X-XX-XXX-XX-XX-XXXXX` / Shop Type dropdown / Pain Point textarea (step 2). Take rate confirmed 5%, **no contract**, equipment minimum = phone + tripod (tripod ships in welcome kit). Troy began Phase 1 of GHL build mid-session; final state unclear. Split session into 3 parallel workstreams: (A) Facebook Ads scale idea, (B) Dealer Kit sourcing, (C) GHL ↔ landing page wire-up. Three paste-ready session-start prompts saved to `Projects/GunStreamer/Session-Start-Prompts-Three-Streams-2026-05-28.md`. | Public preview URL: `gunstreamer-dealercontact-preview.vercel.app` (random-hash previews are SSO-gated 401). Production target: `gunstreamer.com/dealercontact`. Open threads: A2P 10DLC brand-registration entity (BBGV LLC vs GunStreamer LLC), Shop Type dropdown options, dev-cutover scenario A/B/C, GHL build status confirmation. The `/last30days` research found NO firearms/FFL/dealer-network/wholesale snapshot exists in any marketplace — Troy is adapting the official GHL agency onboarding snapshot. |
| 2026-05-27 | **Dealer Launch Kit — full strategic reset + production system** | Killed Phase 1/2 Crew Pack expansion ($325-475/dealer, tribal-coded) → final $94 9-piece welcoming-party kit. L.I.V.E. method codified as official MO across all GS marketing (Local · Inventory · Value · Event). 50 founding dealers locked, GS eats kit cost in v1 (no dealer co-pay), Heritage aesthetic over Mil-Restraint, forest olive primary palette. **Pilot dealer: Van's Outdoors in Cullman, AL (NOT Brandon, MS — earlier docs are wrong, follow-up to sweep launch-kit/*.md files).** Two-layer design system (Master Prompt + per-dealer Brand Profile JSON) cuts per-dealer design time from 60-90 min → ~10 min. Hats moved to milestone unlock (first-show-complete). 7 production docs in `~/Work/GunStreamer/launch-kit/`: AESTHETIC-DEMO.html, VANS-APPLIED.html, KIT-SPEC-FINAL.md, VENDOR-SOURCING.md, DESIGN-WORKFLOW.md, PRODUCTION-PLAYBOOK.md, IMAGE-GEN-PROMPTS.md. | Vendor stack locked per SKU (MOO/StickerMule/GotPrint/local embroiderer/4imprint/Etsy/Displays2Go/Packlane/Crane&Co). Milestone unlock schedule = $303/dealer/yr total (70%+ cost reduction vs Phase 2's $1,100-1,800). Existing `~/.claude/skills/gunstreamer-dealer-kit/` skill is now superseded — has old 11-piece $200-300 spec, needs update or archive. No firearms visible anywhere in kit (Meta-compliance baked in by design). Blocked on Van's deliverables (antler-V SVG + hosts photos + founding year + first event) and 4 long-pole vendor orders (Packlane 50-unit box run $200, Crane & Co 100-pack cards $50, Displays2Go 10-pack A-frame stands $80, local embroiderer + digitizing $25). Session log: `~/obsidian-vault/Claude-Shared-Brain/sessions/2026-05-27-claude-code-gunstreamer-dealer-launch-kit-production-system.md`. |
| 2026-05-21 | **184K list reality check** | Troy corrected the strategy doc: 184K BBGV-cross-pollinated contacts are unengaged leftovers / bounce magnets / non-responders. Not a viewer-growth asset. Updated §5 System C to STOP further waves. `CONTENT-GROWTH-STRATEGY-2026.md` Phase 1 needs a rewrite — its entire "Activate" phase is built on this fictional list. | Impact: removes the #1 cheap-viewer lever from the playbook. Real top levers are now Meta launch (gated on OAuth + Pixel), Reddit (zero execution), Shorts-from-streams (compounding), and the real BBGV `Signal` list (Phase 1 deliverability recovery still blocking). |
| 2026-05-19 | **Klaviyo discovery — OAuth pivot pending** | While starting OAuth deployment, found that Klaviyo segment `[LIVE AUCTIONS] All Subscribers` (ID: `T7Dqzn`) already runs a daily live-auction reminder to ~30 subscribers via the `gunstreamer-live-auction-notify` scheduled skill. The all-Google Apps Script architecture committed earlier duplicates this. **Decision pending:** pivot OAuth deployment to Klaviyo integration (recommended) vs stay all-Google. Session ended before decision. See `Handoff-Session-End-2026-05-19.md`. | Apps Script doc remains valid as fallback. Klaviyo MCP tools available. The hunter creative prompts haven't been run in ChatGPT yet either. |
